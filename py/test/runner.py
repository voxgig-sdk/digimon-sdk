...
class DigimonTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            DigimonTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = DigimonTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = DigimonTestRunner.getenv("DIGIMON_TEST_LIVE")
        override = DigimonTestRunner.getenv("DIGIMON_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = DigimonTestRunner.getenv(key)
        ...
        explain = DigimonTestRunner.getenv("DIGIMON_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["DIGIMON_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if DigimonTestRunner._test_control is not None:
            return DigimonTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                DigimonTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            DigimonTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = DigimonTestRunner.load_test_control()
...
def load_env_local():
    DigimonTestRunner.load_env_local()


def env_override(m):
    return DigimonTestRunner.env_override(m)


def entity_data(v):
    return DigimonTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return DigimonTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return DigimonTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return DigimonTestRunner.load_test_control()


def live_client_options():
    return DigimonTestRunner.live_client_options()


def live_delay_ms():
    return DigimonTestRunner.live_delay_ms()
