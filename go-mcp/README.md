...
[MCP](https://modelcontextprotocol.io) server exposing the Digimon SDK as
two agent tools — `digimon_list` and `digimon_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/digimon-mcp)
make build

# 2. Provide credentials via the environment
export DIGIMON_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user digimon \
  -- /absolute/path/to/digimon-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./digimon-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// digimon_list: first page of records
{ "entity": "attribute" }
{ "entity": "attribute", "query": { } }

// digimon_load: one record by id
{ "entity": "attribute", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/digimon-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user digimon \
     -- "$PWD"/dist/*/digimon-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `digimon_list` and `digimon_load` tools now appear
   in new sessions. Ask the agent to *"list attribute using digimon"*
   and it calls `digimon_list` with `{"entity":"attribute"}`.
...
```sh
export DIGIMON_APIKEY=sk_live_xxx            # API key
export DIGIMON_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `digimon_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `digimon_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DIGIMON_APIKEY` | API key sent with every request. |
| `DIGIMON_BASE` | Optional override of the API base URL. |

### Entities

The 6 entities valid as the `entity` argument:

attribute | digimon | field | level | skill | type

### Smoke test via HTTP (raw JSON-RPC)

```sh
./digimon-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"digimon_load","arguments":{"entity":"attribute","query":{"id":1}}}}'
```
