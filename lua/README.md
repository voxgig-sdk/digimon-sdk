...
The Lua SDK for the Digimon API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Attribute(nil):list(nil, nil)
...
```lua
local result, err = client:Attribute(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:Digimon(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── digimon_sdk.lua    -- Main SDK module
...
The main module (`digimon_sdk`) exports the SDK constructor
