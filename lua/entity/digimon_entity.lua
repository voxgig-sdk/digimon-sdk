...
local DigimonEntity = {}
DigimonEntity.__index = DigimonEntity


function DigimonEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DigimonEntity)
  self._name = "digimon"
  self._client = client
...
function DigimonEntity:get_name()
  return self._name
end


function DigimonEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DigimonEntity.new(self._client, opts)
end
...
function DigimonEntity:mark_deleted()
  self._deleted = true
end


function DigimonEntity:deleted()
  return true == self._deleted
end


function DigimonEntity:data_set(args)
  if args ~= nil then
...
function DigimonEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DigimonEntity:match_set(args)
  if args ~= nil then
...
function DigimonEntity:match_get()
...
function DigimonEntity:stream(action, args, callopts)
...
function DigimonEntity:_run_op(ctx, post_done)
  local utility = self._utility
