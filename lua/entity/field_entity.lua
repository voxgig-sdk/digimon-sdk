...
local FieldEntity = {}
FieldEntity.__index = FieldEntity


function FieldEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FieldEntity)
  self._name = "field"
  self._client = client
...
function FieldEntity:get_name()
  return self._name
end


function FieldEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FieldEntity.new(self._client, opts)
end
...
function FieldEntity:mark_deleted()
  self._deleted = true
end


function FieldEntity:deleted()
  return true == self._deleted
end


function FieldEntity:data_set(args)
  if args ~= nil then
...
function FieldEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FieldEntity:match_set(args)
  if args ~= nil then
...
function FieldEntity:match_get()
...
function FieldEntity:stream(action, args, callopts)
...
---@param reqmatch FieldLoadMatch
---@param ctrl? table
---@return Field
---@return string? err
function FieldEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch FieldListMatch
---@param ctrl? table
---@return Field[]
---@return string? err
function FieldEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function FieldEntity:_run_op(ctx, post_done)
  local utility = self._utility
