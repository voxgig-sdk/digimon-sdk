...
local TypeEntity = {}
TypeEntity.__index = TypeEntity


function TypeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TypeEntity)
  self._name = "type"
  self._client = client
...
function TypeEntity:get_name()
  return self._name
end


function TypeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TypeEntity.new(self._client, opts)
end
...
function TypeEntity:mark_deleted()
  self._deleted = true
end


function TypeEntity:deleted()
  return true == self._deleted
end


function TypeEntity:data_set(args)
  if args ~= nil then
...
function TypeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TypeEntity:match_set(args)
  if args ~= nil then
...
function TypeEntity:stream(action, args, callopts)
...
function TypeEntity:_run_op(ctx, post_done)
  local utility = self._utility
