...
describe("LevelDirect", function()
  it("should direct-list-level", function()
    local setup = level_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-level", setup.live and "live" or "unit")
  ...
  it("should direct-load-level", function()
    local setup = level_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-level", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function level_direct_setup(mockres)
