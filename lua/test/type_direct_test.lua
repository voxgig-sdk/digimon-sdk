...
describe("TypeDirect", function()
  it("should direct-list-type", function()
    local setup = type_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-type", setup.live and "live" or "unit")
  ...
  it("should direct-load-type", function()
    local setup = type_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-type", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function type_direct_setup(mockres)
