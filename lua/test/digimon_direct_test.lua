...
describe("DigimonDirect", function()
  it("should direct-list-digimon", function()
    local setup = digimon_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-digimon", setup.live and "live" or "unit")
  ...
  it("should direct-load-digimon", function()
    local setup = digimon_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-digimon", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function digimon_direct_setup(mockres)
