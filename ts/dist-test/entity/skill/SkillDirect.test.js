...
(0, node_test_1.describe)('SkillDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-skill', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-skill', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['DIGIMON_TEST_SKILL_ENTID'];
