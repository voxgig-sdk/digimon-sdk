...
(0, node_test_1.describe)('AttributeDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-attribute', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-attribute', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-attribute', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-attribute', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.DIGIMON_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    const calls = [];
    const env = (0, utility_1.envOverride)({
        'DIGIMON_TEST_ATTRIBUTE_ENTID': {},
    ...
    if (live) {
        ...
        const client = new __1.DigimonSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['DIGIMON_TEST_ATTRIBUTE_ENTID'];
    ...
    const client = new __1.DigimonSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=AttributeDirect.test.js.map