...
//     (\`new DigimonSDK(...)\` / \`DigimonSDK.test(...)\` become
//     \`DigimonSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"attribute":{"test01":{"id":"test01"}},"digimon":{"test01":{"id":"test01"}},"field":{"test01":{"id":"test01"}},"level":{"test01":{"id":"test01"}},"skill":{"test01":{"id":"test01"}},"type":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new DigimonSDK(...)\` and any
// \`DigimonSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+DigimonSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/DigimonSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
