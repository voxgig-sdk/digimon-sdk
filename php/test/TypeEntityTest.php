...
class TypeEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Type(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Type(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = type_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "type." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set DIGIMON_TEST_TYPE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $type_ref01_data = null;
        if (count($type_ref01_data_raw) > 0) {
...
function type_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/type/TypeTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["type01", "type02", "type03"] as $k) {
    ...
    $entid_env_raw = getenv("DIGIMON_TEST_TYPE_ENTID");
