...
class FieldEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new FieldEntity($this->_client, $opts);
    }

    /**
     * @param Field|array $args Field data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = DigimonHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Field|array The current Field data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Field fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = DigimonHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Field fields).
     */
     ...
     * Load a single Field.
     *
     * @param FieldLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed FieldLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Field|array The loaded Field as an assoc-array at the
     *   SDK boundary; throws DigimonError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = DigimonHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List Field items matching the given filter.
     *
     * @param FieldListMatch|array|null $reqmatch Match filter (any subset
     *   of Field fields) as an assoc-array; FieldListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Field[]|array A list of Field items as assoc-arrays at
     *   the SDK boundary; throws DigimonError on failure (item-5 convention).
     */
