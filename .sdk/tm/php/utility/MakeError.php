...
class DigimonMakeError
{
    public static function call(?DigimonContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new DigimonContext([], null);
        }
        $op = $ctx->op ?? new DigimonOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new DigimonResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof DigimonError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new DigimonError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof DigimonError) {
