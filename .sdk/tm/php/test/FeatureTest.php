...
class FtCtrl extends DigimonControl
{
...
class FtClient
{
    ...
    private DigimonUtility $utility;

    public function __construct(DigimonUtility $utility, array $options)
    {
    ...
    public function get_utility(): DigimonUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DigimonContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DigimonContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DigimonConfig::make_feature($name);
    ...
    private static function buildUrl(DigimonSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DigimonError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DigimonResponse($fetched);
        }
    ...
    private function fail(DigimonContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DigimonContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
