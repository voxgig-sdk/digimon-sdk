...
{
    private static function utility(): DigimonUtility
    {
        return new DigimonUtility();
    }
    ...
    private static function ctx(array $over = []): DigimonContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new DigimonContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new DigimonOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof DigimonError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new DigimonOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = DigimonMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new DigimonOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = DigimonMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new DigimonOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = DigimonMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DigimonSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = DigimonMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new DigimonResponse([]),
            'result' => new DigimonResult([]),
        ]);
        [, $err] = DigimonMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new DigimonSpec([]),
            'response' => null,
            'result' => new DigimonResult([]),
        ]);
        [, $err] = DigimonMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new DigimonSpec(['step' => 's']),
            'response' => new DigimonResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new DigimonResult([]),
        ]);
        [, $err] = DigimonMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new DigimonSpec(['step' => 's']),
            'response' => new DigimonResponse(self::resp(200, ['v' => 1])),
            'result' => new DigimonResult([]),
        ]);
        [, $err] = DigimonMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new DigimonSpec(['step' => 's']),
            'response' => new DigimonResponse(self::resp(200, ['v' => 2])),
            'result' => new DigimonResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        DigimonMakeResponse::call($ctx);
    ...
    {
        $preset = new DigimonResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new DigimonSpec([]),
            'response' => new DigimonResponse([]),
            'result' => new DigimonResult([]),
        ...
        [$got, $err] = DigimonMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new DigimonResult([])]);
        [, $err] = DigimonMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new DigimonSpec([]), 'result' => null]);
        [, $err] = DigimonMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DigimonSpec(['step' => 's']),
            'result' => new DigimonResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new DigimonOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DigimonMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DigimonSpec(['step' => 's']),
            'result' => new DigimonResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new DigimonOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DigimonMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DigimonResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new DigimonSpec([]),
            'result' => new DigimonResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = DigimonMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DigimonMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new DigimonError('boom', 'boom');
        $utility->fetcher = function (DigimonContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DigimonSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = DigimonMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (DigimonContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = DigimonMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(DigimonResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (DigimonContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DigimonSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = DigimonMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new DigimonResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new DigimonSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = DigimonMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = DigimonMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = DigimonMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new DigimonResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, DigimonDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new DigimonResult(['ok' => false])]);
        $this->expectException(DigimonError::class);
        DigimonDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new DigimonResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', DigimonMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        DigimonMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new DigimonBaseFeature();
        $b = new DigimonBaseFeature();
        DigimonFeatureAdd::call($ctx, $a);
        DigimonFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): DigimonBaseFeature
    {
        $f = new DigimonBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        DigimonFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): DigimonContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new DigimonSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DigimonPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new DigimonResponse(['headers' => null]),
            'result' => new DigimonResult([]),
        ]);
        DigimonResultHeaders::call($ctx);
