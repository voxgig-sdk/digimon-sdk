...
module DigimonUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = DigimonResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DigimonResponse.new(fetched)
    else
