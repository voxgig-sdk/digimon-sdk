...
boru-driven command-line client **and** interactive REPL for the Digimon
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/digimon-cli)
make build
...
./digimon-cli list attribute
./digimon-cli load 1 attribute            # {id:1} shorthand
./digimon-cli load '{id:1}' attribute       # explicit match map
./digimon-cli list digimon

# 5. Override the API base URL for a single call
DIGIMON_BASE=https://api.example.com ./digimon-cli list attribute

# 6. No arguments -> interactive REPL
./digimon-cli
digimon> list attribute
digimon> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/digimon-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/digimon-cli list attribute
   ```
...
```sh
./digimon-cli load 1 attribute          # scalar shorthand for {id:1}
./digimon-cli load '{id:1}' attribute     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `digimon>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `attribute`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/digimon-cli`. |
...
(`./digimon-cli <expr>`) and interactively (the REPL), and expressions compose the
