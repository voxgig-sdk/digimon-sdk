// Command Digimon-cli is a CLI + REPL for the Digimon SDK,
...
const prompt = "digimon"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "attribute digimon field level skill type"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      DIGIMON_APIKEY (api key), DIGIMON_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: DIGIMON_APIKEY carries the API key and
	// DIGIMON_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("DIGIMON_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("DIGIMON_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewDigimonSDK(opts)
