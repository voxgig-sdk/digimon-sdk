...
import (
	...
	omni "github.com/voxgig-sdk/digimon-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DigimonError); is {
				out := map[string]any{
					"name":    "DigimonError",
...
func omniCtx(arg any, client *sdk.DigimonSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DigimonSDK); is {
			client = live
