...
func TestAttributeEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		// LOAD
		attributeRef01MatchDt0 := map[string]any{
			"id": attributeRef01Data["id"],
		}
		attributeRef01DataDt0Loaded, err := attributeRef01Ent.Load(attributeRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		attributeRef01DataDt0LoadResult := core.ToMapAny(entityData(attributeRef01DataDt0Loaded))
		if attributeRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if attributeRef01DataDt0LoadResult["id"] != attributeRef01Data["id"] {
...
func attributeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DIGIMON_TEST_ATTRIBUTE_ENTID")
