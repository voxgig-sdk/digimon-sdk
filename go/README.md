...
The Golang SDK for the Digimon API. Provides an entity-oriented interface using standard Go conventions — no generics required, data flows as `map[string]any`.


## Install
```bash
go get github.com/voxgig-sdk/digimon-sdk
```
...
```bash
go mod edit -replace github.com/voxgig-sdk/digimon-sdk=../path/to/github.com/voxgig-sdk/digimon-sdk
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *DigimonSDK
```
...
| `Skill` | `(data map[string]any) DigimonEntity` | Create a Skill entity instance. |
| `Type` | `(data map[string]any) DigimonEntity` | Create a Type entity instance. |

### Entity interface (DigimonEntity)

All entities implement the `DigimonEntity` interface.
...
Create an instance: `digimon := client.Digimon(nil)`
...
Create an instance: `field := client.Field(nil)`
...
Create an instance: `level := client.Level(nil)`
...
```
github.com/voxgig-sdk/digimon-sdk/
├── digimon.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/digimon-sdk`) re-exports everything needed
