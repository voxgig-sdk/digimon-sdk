...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = DigimonOmni.make_runner(TEST_JSON_FILE, DigimonSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : DigimonOmni.errify(err)
    end
