...
class DigimonEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = DigimonSDK.test(seed, nil)
    seen = base.Digimon(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Digimon(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = digimon_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "digimon." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set DIGIMON_TEST_DIGIMON_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    digimon_ref01_data = nil
    if digimon_ref01_data_raw.length > 0
      digimon_ref01_data = Helpers.to_map(digimon_ref01_data_raw[0][1])
    end

    # LIST
    digimon_ref01_ent = client.Digimon(nil)
    digimon_ref01_match = {}

    digimon_ref01_list_result = digimon_ref01_ent.list(digimon_ref01_match, nil)
    assert digimon_ref01_list_result.is_a?(Array)
...
def digimon_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "digimon", "DigimonTestData.json")
  ...
  entid_env_raw = ENV["DIGIMON_TEST_DIGIMON_ENTID"]
