...
class FieldEntity
  ...
  def make
    opts = @_entopts.dup
    FieldEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = DigimonHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Field, Hash] the current Field data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = DigimonHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Field fields)
  def match_get
  ...
  # Load a single Field.
  #
  # @param reqmatch [FieldLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Field.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Field, Hash] the loaded Field; raises DigimonError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = DigimonHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [FieldListMatch, Hash, nil] match filter (any subset of
  #   Field fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Field>, Array] the matching Field items; raises DigimonError on failure
