...
class DigimonEntity
  ...
  def make
    opts = @_entopts.dup
    DigimonEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.Digimon.load works with no args).
  ...
  #
  # @param reqmatch [DigimonListMatch, Hash, nil] match filter (any subset of
  #   Digimon fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
